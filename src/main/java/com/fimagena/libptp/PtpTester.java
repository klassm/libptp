...
import com.fimagena.libptp.PtpDataType.ObjectFormatCode;
...
public class PtpTester {
  ...
  public static void main(String[] args) throws Exception {
    ...
    InetAddress ip = InetAddress.getByName("192.168.1.1");  // adjust IP
    if (args.length > 0) {
    ...
    try {
      ...
      StorageID allStorageId = new StorageID(0xFFFFFFFF);

      System.out.println("Num objects");
      System.out.println(session.getNumObjects(allStorageId));

      System.out.println("GetStorageInfo: " + allStorageId.mValue);
      PtpDataType.StorageInfoDataSet storageInfo = session.getStorageInfo(allStorageId);
      System.out.println(storageInfo.toString());

      System.out.println("GetObjectHandles:");
      PtpDataType.ObjectHandle[] objectHandles = session
          .getObjectHandles(allStorageId, new ObjectFormatCode(0x3000));
      ...
      File baseDir = new File("/tmp/images");
      baseDir.mkdir();
      ...
      for (PtpDataType.ObjectHandle objectHandle : objectHandles) {
        ...
        byte[] data = session.getObject(objectHandle);
        try (FileOutputStream stream = new FileOutputStream(new File(baseDir, objectInfo.mFilename.mString))) {
          stream.write(data);
        }
      }
